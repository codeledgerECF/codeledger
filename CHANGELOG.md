...
- **Readiness evidence honesty** — AI-native readiness now checks runbook and known-issue evidence in addition to the existing dimensions. Repos without those artifacts may show lower evidence coverage or `not_evaluable`/`degraded` fields until teams add the evidence; this is intentional missing-evidence disclosure, not an optimistic pass.
